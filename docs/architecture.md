...
For fish-backed wrapped submit flows, the service layer now treats the binary as the runtime source of truth for the canonical wrapper. The repo-owned fish wrapper source is embedded into the binary, existing fish definitions are validated via the wrapper hash contract, and missing or stale wrappers are lazily bootstrapped into the target shell before execution. Once a pane has the canonical wrapper loaded, repeated wrapped commands reuse it without resending the definition on every command. The wrapper still preserves daemon-owned interaction markers internally, supports user preview via `-p`, and falls back to the legacy inline wrapper path if validation, bootstrap, or clean invocation still fails.
