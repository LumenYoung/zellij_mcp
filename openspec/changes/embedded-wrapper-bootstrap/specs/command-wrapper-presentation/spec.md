...
#### Scenario: Human-visible wrapped command submission with canonical wrapper already loaded
- **WHEN** the daemon submits a wrapped shell command to a human-visible fish pane that already has the canonical wrapper definition loaded
- **THEN** the pane shows a compact wrapper command rather than the full inline `printf`/`begin` instrumentation script

#### Scenario: Human-visible wrapped command submission after lazy bootstrap
- **WHEN** the daemon submits a wrapped shell command to a human-visible fish pane that is missing the canonical wrapper definition
- **THEN** the daemon bootstraps the canonical wrapper and the pane still uses the compact wrapper command for execution instead of immediately degrading to the inline instrumentation script
